...
const VideoUploader: React.FC<VideoUploaderProps> = ({ onFileSelect }) => {
  ...
  return (
    <div 
      className={`relative w-full h-80 rounded-2xl border-2 border-dashed transition-all duration-300 flex flex-col items-center justify-center p-8 cursor-pointer
    ...
    >
      ...
      <div className="w-16 h-16 mb-6 rounded-full bg-slate-800 flex items-center justify-center text-brand-400 shadow-lg shadow-black/20">
        <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
        </svg>
      </div>

      <h3 className="text-xl font-semibold text-white mb-2">동영상 업로드</h3>
      ...
      <Button 
        ...
        variant="outline"
        className="bg-dark-900 border-brand-500/30 text-brand-400 hover:bg-brand-500 hover:text-white hover:border-transparent"
      >
        파일 선택하기
      </Button>
    </div>
